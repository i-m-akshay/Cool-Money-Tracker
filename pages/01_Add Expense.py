...
if not os.path.exists(
        folder_path):  # if no folder named data ,then make data folder
    os.makedirs(folder_path)

if not os.path.exists(excel_file_path):  # if no file named expense.csv
    expenses = pd.DataFrame(
        columns=["Date", "Category", "Description", "Currency Type", "Amount"])
...
def insert(date, category, description, currency_type, amount):
    ...
    if description != "" and amount > 0:
        ...
        dataframe.loc[length] = [
            date, category, description, currency_type, amount
        ]
        ...
        st.error(
            'Please provide a description and a valid amount value greater than zero.',
            icon="🚨")
...
category = st.selectbox(
    "Category:card_index_dividers:",
    ("Housing", "Utilities", "Transportation", "Food", "Healthcare",
     "Insurance", "DebtPayments", "Entertainment", "PersonalCare", "Education",
     "Savings", "Taxes", "Miscellaneous"),
    key="cat")

description = st.text_input('Description :flashlight:', key='desc')

currency_type = st.selectbox("Currency type :heavy_dollar_sign: /  :euro:",
                             ("Dollars", "Euros"))

amount = st.number_input('Amount :money_mouth_face:',
                         key='am',
                         min_value=0,
                         step=1,
                         max_value=20000)
